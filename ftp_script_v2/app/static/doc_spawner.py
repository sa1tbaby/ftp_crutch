...
def print_status(iterate, doc_count):
    if int(iterate % (doc_count / 10)) == 0:
        status_message = '__________ ~ XX%'

        status = int(iterate // (doc_count / 100))

        status_message = status_message.replace('_', '#', 1)

        os_system('cls')

        return status_message.replace('XX', str(status))
...
for iterate in range(doc_count):
    ...
    print(print_status(iterate, doc_count))
